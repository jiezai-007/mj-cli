...
class Creator {
    constructor(name,context){
        ...
        this.pm = null;
        //预设提示选项  用户输入提示
        this.presetPrompt = this.resolvePresetPrompt()
        // 自定义特性提示选项（复选框）
    ...
    resolvePresetPrompt(){
        const presetChoices = Object.entries(defaults.presets).map(([name,preset])=>{
            return {
                name:`${name}(${Object.keys(preset.plugins).join(',')})`,//将预设插件防盗提示框
                value: name
            }
        })
        return {
    ...
    async initPackageManagerEnv(preset) {
      ...
      deps.forEach(dep => {
        ...
        pkg.devDependencies[dep] = version //获取包的版本号
      })
    ...
    run (command, args) { 
      ...
      return execa(command, args, { cwd: this.context }) //执行命令
    }
